...
/// The backdrop for Direction A: seven planes between the child and the horizon.
///
...
/// illustrator builds it: each plane further away is lower in contrast, lower
/// in saturation and closer to the horizon's warm haze, and the two nearest
/// planes are near-black silhouettes so the eye has something to sit behind.
///
/// `horizon` is a fraction of height. On a phone the horizon sits higher, which
/// gives the beach more room and stops the sign from crowding the sea.
public struct StorybookWorld: View {
    let p: StorybookPalette
    let horizon: CGFloat
    let showForeground: Bool

    public init(_ p: StorybookPalette = .noon, horizon: CGFloat = 0.46,
    ...
    public var body: some View {
        Canvas { ctx, size in
            ...
            // 1. Sky. Warm at the horizon, cool at the top -- the reverse of
            //    the usual UI gradient, and the whole reason the scene reads as
            //    late afternoon instead of as a header.
            ...
            // 2. Sun, low and to the left. Everything drawn later is lit from
            //    this direction; that consistency is what a "painterly" read is
            //    actually made of.
            // Kept low and left, below the HUD: a sun disc behind a health bar
            // washes the bar out, which the first pass proved by doing it.
            let sun = CGPoint(x: W * 0.15, y: hy * 0.74)
            for i in stride(from: 14, through: 1, by: -1) {
                let r = hy * 0.085 * CGFloat(i)
                ctx.fill(Path(ellipseIn: CGRect(x: sun.x - r, y: sun.y - r,
                                                width: r * 2, height: r * 2)),
                         with: .color(p.sunHalo.opacity(0.05)))
            }
            // The disc itself is small and soft-edged. A hard bright circle in
            // a flat sky reads as a bug, not as the sun.
            let sr = hy * 0.085
            ...
            // 3. Two cloud banks, flat-bottomed and warm-lit underneath.
            ...
            // 6. Sea: three bands, darkest at the horizon, plus a sun path and
            //    a foam line where it meets the sand.
            ...
            // Sun path on the water: short horizontal dashes narrowing upward.
            for i in 0..<9 {
                let t = CGFloat(i) / 8
            ...
            for i in 0..<7 {
                ...
                let crest = Path.smoothOpen([
                    CGPoint(x: x, y: y),
                    CGPoint(x: x + w * 0.5, y: y - (H - hy) * 0.012),
                    CGPoint(x: x + w, y: y)
                ])
                ctx.stroke(crest, with: .color(p.foam.opacity(0.42)),
            ...
            ctx.fill(wet, with: .linearGradient(
                Gradient(colors: [p.sandFar.mixed(with: p.seaShallow, by: 0.30),
                                  p.sandFar]),
                startPoint: CGPoint(x: 0, y: beach),
                endPoint: CGPoint(x: 0, y: beach + (H - hy) * 0.22)))
            // Foam edge riding the same curve.
            var foamLine = Path()
            ...
            // Scattered grain and a few shells so the sand is not a flat fill.
            ...
            // 8. Foreground silhouettes. Near-black, bottom corners only, so
            //    the play area is never covered.
            if showForeground {
                // Rooted well off-canvas and kept small: foliage that crosses a
                // creature's feet reads as damage, not as depth.
                frondCluster(&ctx, origin: CGPoint(x: -W * 0.05, y: H + H * 0.05),
                             scale: min(W, H) * 0.30, flip: false,
                             color: p.foliageNear)
            ...
            // 9. A warm light wash from the sun corner and a soft vignette.
            //    One unified light is what stops eight drawn layers from
            //    looking like eight stickers.
            ctx.fill(Path(CGRect(origin: .zero, size: size)),
                     with: .radialGradient(
                        Gradient(colors: [p.sunHalo.opacity(0.20), .clear]),
                        center: sun, startRadius: 0, endRadius: max(W, H) * 0.72))
            ctx.fill(Path(CGRect(origin: .zero, size: size)),
                     with: .radialGradient(
                        Gradient(stops: [
                            .init(color: .clear, location: 0.55),
                            .init(color: p.ink.opacity(0.20), location: 1)
                        ]),
    ...
    private func drawCloud(_ ctx: inout GraphicsContext, at c: CGPoint, w: CGFloat, h: CGFloat) {
        ...
        let path = Path.smoothClosed(lobes, tension: 0.62)
    ...
    private func palmSilhouette(_ ctx: inout GraphicsContext, base: CGPoint,
        ...
        let trunk = Path.smoothOpen([
            base,
            CGPoint(x: base.x + h * lean, y: base.y - h * 0.5),
            top
        ])
        ctx.stroke(trunk, with: .color(color),
                   style: StrokeStyle(lineWidth: max(2, h * 0.075), lineCap: .round))
        for i in 0..<5 {
            let a = -CGFloat.pi * 0.92 + CGFloat(i) * .pi * 0.21
            let tip = CGPoint(x: top.x + cos(a) * h * 0.52,
                              y: top.y + sin(a) * h * 0.34)
            let frond = Path.smoothOpen([
                top,
                CGPoint(x: (top.x + tip.x) / 2, y: (top.y + tip.y) / 2 - h * 0.10),
                tip
            ])
            ctx.stroke(frond, with: .color(color),
    ...
    /// leaf blades rather than one blob, so the silhouette has teeth.
    private func frondCluster(_ ctx: inout GraphicsContext, origin: CGPoint,
        ...
        let angles: [CGFloat] = [-1.42, -1.10, -0.80, -0.52, -0.26]
        for (i, a0) in angles.enumerated() {
            let a = a0
            ...
            // Blade: spine out, wide belly, spine back.
            let spine = Path.smoothOpen([origin, mid, tip])
            let blade = Path.smoothClosed([
                origin,
            ...
            ctx.stroke(spine, with: .color(color.mixed(with: .white, by: 0.10)),
