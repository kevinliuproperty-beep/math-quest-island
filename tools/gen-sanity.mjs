...
/* INTEGRATOR (wave 1): the two lanes seeded the same stream two ways. The decimals
   lane hands a DELEGATING Math to the vm (below), which seeds every generator, the
   feed and every ruler that samples through the vm. The p3numbers lane seeded the
   HOST `Math.random` instead, because several of its negative controls rebuild a
   frozen option set OUTSIDE the vm and call `Math.random` directly - those go
   non-deterministic unless the host is seeded too, and step 2 of this integration
   requires two verbatim runs to be byte-identical. Both are kept: one stream,
   consumed through SeededMath inside the vm and through Math outside it. */
Math.random = rnd;
...
/* Return: null = verified, string = failure, false = no oracle matched.
   ...
   exactly two blocks - the P3 fractions bank added by the 2026-09-15 depth sweep,
   and the decimals bank below - so that a bank of house stem shapes cannot claim
   another topic's stems, and another topic's looser branch cannot claim the bank's.
   Everything else here still dispatches on the rendered text alone. */
...
const DEC_RANK_DRAWS = Number(process.env.RANK_SAMPLES || 2000);
...
const DEC_RANK_FLOOR = 200;                 /* below this many measurable draws, no verdict */
...
let rankReport = null;
{
  ...
  rankReport = decRankScan(decGens, DEC_RANK_DRAWS);
  for (const r of rankReport.rows) {
    if (r.exempt || r.n < DEC_RANK_FLOOR) { decRankRows.push({ ...r, ok: true }); continue; }
    const ok = r.worst <= RANK_CEILING && r.thinnest >= RANK_MIN_SHARE;
    decRankRows.push({ ...r, ok, under: r.thinnest < RANK_MIN_SHARE });
    if (!ok) failures++;
  }
}
...
{
  const decGens = GENS.filter(g => g.topic === 'decimals').map(g => ({ name: g.name, fn: g.fn }));
  for (const r of decProseScan(decGens, DEC_RANK_DRAWS)) {
    let note = null;
    ...
    if (note) { decProseRows.push({ ...r, ok: false, note }); failures++; continue; }
    if (r.n < DEC_PROSE_FLOOR) { decProseRows.push({ ...r, ok: true, note: 'not enough prose draws to judge' }); continue; }
    for (let fi = 0; fi < DEC_FEATURES.length; fi++) {
    ...
    decProseRows.push({ ...r, ok: !note, note });
    if (note) failures++;
  }
}
...
/* THE SEVENTH PASS'S GATE FINDING (W2, refutation v7 @ ed2c0dc). The shape key
   ...
   At DEC_RANK_DRAWS = 2000 each of gDecQuotient's five divisor modes holds ~300-500
...
{
  const decGens = GENS.filter(g => g.topic === 'decimals').map(g => ({ name: g.name, fn: g.fn }));
  for (const r of decCouplingScan(decGens, DEC_RANK_DRAWS)) {
    couplingRows.push(r);
    if (!r.ok) failures++;
  }
}
...
if (rankReport) {
  console.log('');
  console.log(`MAGNITUDE-RANK  (${DEC_RANK_DRAWS} draws x ${rankReport.rows.length} decimals generators, ` +
  ...
  const badRank = decRankRows.filter(r => !r.ok);
  if (badRank.length) {
    ...
    const gatedRows = decRankRows.filter(r => !r.exempt && r.n >= DEC_RANK_FLOOR).sort((a, b) => b.worst - a.worst);
    const w = gatedRows[0];
...
if (decProseRows.length) {
  console.log('');
  const judged = decProseRows.filter(r => r.n >= DEC_PROSE_FLOOR);
  console.log(`PROSE-OPTION GATE  (${DEC_RANK_DRAWS} draws x ${decProseRows.length} decimals generators, ${judged.length} of them with ` +
  ...
  const badProse = decProseRows.filter(r => !r.ok);
  if (badProse.length) {
    ...
    const skipped = decProseRows.filter(r => r.n >= DEC_PROSE_FLOOR && r.lenN < DEC_PROSE_FLOOR).map(r => r.name);
    ...
    const modalWorst = decProseRows.filter(r => r.allN >= DEC_PROSE_FLOOR).sort((a, b) => b.modalShare - a.modalShare)[0];
...
if (couplingRows.length) {
  ...
  console.log(`STEM-OPTION COUPLING  (${DEC_RANK_DRAWS} draws x ${couplingRows.length} decimals generators, ` +
