...
// Moved here from `MQQuest.QUnits` on the rehearsal fix pass of 2026-09-07,
// alongside `MQTypedEntry`, so the Patchwerk arena can offer the same unit chips
// the Quest battle does. See the header of `MQTypedEntry.swift` for the
// measurement that forced the move. `MQQuest` keeps `typealias QUnits = MQUnits`.
...
/// `MQUnitChipRow`.
public enum MQUnitClass: String, Sendable, CaseIterable {
...
public enum MQUnits {
    ...
    public static let byClass: [MQUnitClass: [String]] = [
        .length:  ["mm", "cm", "m", "km"],
        .area:    ["cm²", "m²"],
        .volume:  ["ml", "l", "cm³", "m³"],
        .mass:    ["g", "kg"],
    ...
    public static func unitClass(_ unit: String) -> MQUnitClass? {
    ...
    ///
    /// Quest Refutation K3: `MQUnits.table["ml"]` offered `cm³` as a wrong-unit
