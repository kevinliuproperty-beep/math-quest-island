...
/// `QKeypadPolicyTests` re-runs that measurement against the live engine and
...
public struct QKeypadPolicy: Sendable, Equatable {
    /// `.` - decimal answers.
    ...
    public static let digitsOnly = QKeypadPolicy()
    ...
    /// question needs. `QKeypadPolicyTests` asserts every live topic has a row
    /// here AND that no row enables a key its topic never produces.
    public static let byTopic: [String: QKeypadPolicy] = [
        // typed, decimals
        "p3money":     QKeypadPolicy(decimal: true),
        "p5decimals":  QKeypadPolicy(decimal: true),
        "p5rate":      QKeypadPolicy(decimal: true),
        // typed, decimals and fractions
        "p5fractions": QKeypadPolicy(decimal: true, fraction: true),
        // typed, whole numbers
        "p3divide":    .digitsOnly,
    ...
    public static let unmeasuredDefault = QKeypadPolicy(decimal: true)

    public static func forTopic(_ topic: String) -> QKeypadPolicy {
...
/// wrong.
public struct QTypedEntry: Equatable, Sendable {
    ...
    public mutating func press(_ key: Key, policy: QKeypadPolicy) {
        switch key {
        case .digit(let d):
    ...
    public func isEnabled(_ key: Key, policy: QKeypadPolicy) -> Bool {
        var copy = self
