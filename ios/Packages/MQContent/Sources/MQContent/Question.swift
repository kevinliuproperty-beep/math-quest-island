...
public struct Verdict: Codable, Hashable, Sendable {
    ...
    /// learned a new word.
    ///
    /// **Copied verbatim from `feat/unit-sweep` @ `ff487a3`** so the merge is a no-op.
    /// The engine ON THIS BRANCH does not emit `wrong-unit` / `wrong-value` yet - it
    /// emits the single string `"wrong value or unit"`, which lands here as
    /// `.other("wrong value or unit")`. Consumers must not re-derive the split from
    /// `expectedText`; see `MQProgress.WrongReason.wrongValueOrUnit`.
